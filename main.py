...
def nadajFigure(karta):
    ...
    karta.figura = naj_figura

def nadajKolor(karta):
    img = karta.img
...
def card_contouring(filepath):
    ...
    for i, c in enumerate(contours):
        ...
        karty.append(Card(c, img_cropped))
...
def process_selected_images(path, selected_images):
    for filename in selected_images:
        karty = card_contouring(path + filename)
    for card in karty:
        nadajFigure(card)
        nadajKolor(card)
...
if __name__ == '__main__':
    ...
    selected_images = ['four_of_kind.jpg']
    process_selected_images(PATH, selected_images)
    # FIXME: processes only the first element in the list, due to cv.waitKey(0) and cv.destroyAllWindows()
